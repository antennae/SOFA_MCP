...
- `displacement_series` — per-MO max displacement **per step** (length = `steps`; a `null` entry marks a step where that MO went non-finite). Read this when the symptom is "healthy until step N then detonates" — the scalar `max_displacement_per_mo` hides that shape, but the series shows the flat-then-spike trajectory and the exact blowup step
...
| `solver_iter_cap_hit` carries `steps_hit_cap: [...]` | Converges then drifts after step N | Constraint solver hit `maxIterations` — several causes, so look at the logs before acting. If `delta`/`lambda` go NaN while element volumes stay healthy, the compliance matrix W is ill-conditioned: `query_sofa_component` the `*ConstraintCorrection` for a stabilizing field (e.g. `regularizationTerm`, which adds ε·I to W). Otherwise it's redundant constraints in the same subtree, a too-stiff system, or a too-loose `tolerance` — raise the cap or tighten `tolerance`. |
