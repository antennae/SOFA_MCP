...
def createScene(rootNode):
    ...
    # Chambers
    for i in range(3):
        chamberNode = prostateNode.addChild(f"chamber_{i}")
        chamberNode.addObject("MeshSTLLoader", name=f"loader_{i}", filename=f"meshes/inner_{i}.stl")
        chamberNode.addObject("TriangleSetTopologyContainer", name=f"container_{i}", src=f"@loader_{i}")
        chamberNode.addObject("MechanicalObject", name="mo", src=f"@loader_{i}")
        # Pressurization: initially 0.0
        chamberNode.addObject("SurfacePressureConstraint", name="pressure", value=[0.0], triangles=f"@container_{i}.triangles")
        chamberNode.addObject("BarycentricMapping")

    # Visual model
    visualNode = prostateNode.addChild("visual")
    visualNode.addObject("OglModel", name="visualModel", src="@../loader", color=[0.8, 0.4, 0.4, 0.5])
