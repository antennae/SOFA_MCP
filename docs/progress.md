...
## M7 closed; Claude Code does not opt into MCP tasks (2026-09-03)

`runs_in_flight` entries now carry `as_task` / `task_key`, read from docket's `current_execution` contextvar (the FastMCP request context inside a worker is a restored snapshot and cannot answer the opt-in question). Calling `diagnose_scene` from Claude Code on the trunk scene logged `as_task: False`: Claude Code negotiates protocol 2026-07-28 but sends no tasks-extension capability, so the optional-mode tools run blocking for it. The FastMCP client path is verified as a real task. Also observed: Claude Code's stateless HTTP connection survives a server restart without `/mcp`, and its cached tool descriptions do not refresh on reconnect.
